window.__i18n={
  "ja": {
    "siteTitle": "すばらしきSteamゲームの本棚",
    "siteDesc": "Dr.スカラとことりが選んだゲームを並べた棚。セールと価格は毎日自動で更新しています。",
    "lastUpdate": "最終更新",
    "titles": "タイトル",
    "loading": "読み込み中...",
    "onSale": "セール中！",
    "gameList": "イチオシのゲーム",
    "articles": "記事",
    "search": "検索",
    "searchPlaceholder": "ゲーム名で検索...",
    "genre": "ジャンル",
    "genreAll": "すべて",
    "tags": "タグ",
    "display": "表示",
    "sale": "セール",
    "saleAll": "セール中のみ",
    "sale90": "90%以上OFF",
    "sale80": "80%以上OFF",
    "sale70": "70%以上OFF",
    "sale60": "60%以上OFF",
    "sale50": "50%以上OFF",
    "sale40": "40%以上OFF",
    "sale30": "30%以上OFF",
    "sale20": "20%以上OFF",
    "sale10": "10%以上OFF",
    "onlyArticle": "記事ありのみ",
    "sort": "並び替え",
    "sortName": "タイトル順",
    "sortPriceAsc": "価格が安い順",
    "sortPriceDesc": "価格が高い順",
    "sortReview": "レビュー順",
    "sortDiscount": "割引率順",
    "showing": "件表示",
    "clearFilter": "フィルター解除",
    "noResults": "条件に一致するゲームがありません。",
    "viewOnSteam": "Steamで見る",
    "readArticle": "記事を読む",
    "free": "無料",
    "languages": "対応言語",
    "ageRestricted": "年齢制限コンテンツ",
    "ageConfirm": "このゲームは年齢制限があります。表示しますか？",
    "ageYes": "はい、表示する",
    "ageNo": "いいえ",
    "ageBadge": "R18",
    "recommendAll": "みんなにおすすめ",
    "recommendNiche": "同好の士におすすめ",
    "recommendMulti": "マルチ推奨",
    "comingSoon": "私の楽しみ",
    "comingSoonBadge": "未リリース",
    "wishlist": "ウィッシュリストに入れる",
    "freeGames": "フリーゲーム",
    "freeGamesBadge": "無料",
    "featured": "直近のマイベスト！",
    "featuredReadArticle": "記事を読む",
    "tools": "おすすめツール",
    "toolsBadge": "ツール",
    "loadError": "データの読み込みに失敗しました",
    "kicker": "Steam Game Shelf",
    "statUpdated": "最終更新",
    "statShelf": "棚のゲーム",
    "statSale": "セール中",
    "statTools": "ツール",
    "unit": "本",
    "filterButton": "絞り込み",
    "showMore": "もっと見る",
    "remaining": "残り",
    "curators": "選者",
    "saleHint": "いま値下がりしている本棚のゲーム",
    "featuredSteam": "Steamで見る",
    "about": "このサイトについて",
    "sisterSites": "ほかの棚"
  },
  "en": {
    "siteTitle": "The Wonderful Steam Game Shelf",
    "siteDesc": "A shelf of Steam games picked by Dr. Scala and Kotori. Sales and prices update every day.",
    "lastUpdate": "Last updated",
    "titles": "titles",
    "loading": "Loading...",
    "onSale": "On Sale!",
    "gameList": "Our Picks",
    "articles": "Articles",
    "search": "Search",
    "searchPlaceholder": "Search by title...",
    "genre": "Genre",
    "genreAll": "All",
    "tags": "Tags",
    "display": "Filter",
    "sale": "Sale",
    "saleAll": "On sale only",
    "sale90": "90%+ off",
    "sale80": "80%+ off",
    "sale70": "70%+ off",
    "sale60": "60%+ off",
    "sale50": "50%+ off",
    "sale40": "40%+ off",
    "sale30": "30%+ off",
    "sale20": "20%+ off",
    "sale10": "10%+ off",
    "onlyArticle": "With article only",
    "sort": "Sort",
    "sortName": "By title",
    "sortPriceAsc": "Price: low to high",
    "sortPriceDesc": "Price: high to low",
    "sortReview": "By review score",
    "sortDiscount": "By discount",
    "showing": " shown",
    "clearFilter": "Clear filters",
    "noResults": "No games match the current filters.",
    "viewOnSteam": "View on Steam",
    "readArticle": "Read article",
    "free": "Free",
    "languages": "Languages",
    "ageRestricted": "Age-restricted content",
    "ageConfirm": "This game has age-restricted content. Do you want to view it?",
    "ageYes": "Yes, show me",
    "ageNo": "No",
    "ageBadge": "R18",
    "recommendAll": "Recommended for everyone",
    "recommendNiche": "For fellow enthusiasts",
    "recommendMulti": "Best with friends",
    "comingSoon": "Looking Forward To",
    "comingSoonBadge": "Coming Soon",
    "wishlist": "Add to Wishlist",
    "freeGames": "Free Games",
    "freeGamesBadge": "Free",
    "featured": "My Recent Best!",
    "featuredReadArticle": "Read article",
    "tools": "Recommended Tools",
    "toolsBadge": "Tool",
    "loadError": "Failed to load data",
    "kicker": "Steam Game Shelf",
    "statUpdated": "Updated",
    "statShelf": "On the shelf",
    "statSale": "On sale",
    "statTools": "Tools",
    "unit": "",
    "filterButton": "Filters",
    "showMore": "Show more",
    "remaining": "left",
    "curators": "Picked by",
    "saleHint": "Games from our shelf that are discounted right now",
    "featuredSteam": "View on Steam",
    "about": "About",
    "sisterSites": "Other shelves"
  },
  "reviewScoreJa": {
    "Overwhelmingly Positive": "圧倒的に好評",
    "Very Positive": "非常に好評",
    "Positive": "好評",
    "Mostly Positive": "やや好評",
    "Mixed": "賛否両論",
    "Mostly Negative": "やや不評",
    "Negative": "不評",
    "Very Negative": "非常に不評",
    "Overwhelmingly Negative": "圧倒的に不評"
  }
};